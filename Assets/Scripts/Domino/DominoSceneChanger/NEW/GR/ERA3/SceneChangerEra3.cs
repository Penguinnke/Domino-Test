...
public class SceneChangerEra3 : MonoBehaviour
{
        ...
        foreach (string tag in _dominoTagTRUE)
        {
            if (_collision.gameObject.tag == tag)
            {
                    if (_collision.gameObject.tag == tag)
                    {
                        ...
                        {
                            Debug.Log("95% Scene not changed Area3");
                        }
    ...
    {
        // Access the shared list from the SharedSceneListEra3 for this era script
        List<string> scenes = SharedSceneListEra3.Instance.Scenes;
